ShopSlot = InventorySlot.$extend('ShopSlot', {
	__init__: function(mode) {
		...
		self.focused_stat = ko.observable('Glow');
		self.statId = 0;
	},
	__keys__: function() {
	...
	next_stat: function() {
		var self = this;
		self.statId = (self.statId + 1) % game.unlocked_stats_list().length;
		self.focused_stat(game.unlocked_stats_list()[self.statId]);
		self.restock();
	},
	prev_stat: function() {
		var self = this;
		self.statId -= 1;
		if (self.statId<0){
			self.statId = game.unlocked_stats_list().length-1;
		}
		self.focused_stat(game.unlocked_stats_list()[self.statId]);
	...
	generate_part: function() {
		...
		if(self.mode() === 'fractal') {
			...
			var stat_used = self.focused_stat();
			if(!stat_used) {
