...
ko.bindingHandlers.shell_render = {
	init: function(element, valueAccessor) {
		...
		var $elem = $(element);
		$elem.data('shell', shell);
		if(shell.savedStyle)
			element.style = shell.savedStyle;
		if(shell.fade_in() || shell.zoom_in())
		{
			$elem.toggle(false);
			$elem.css('opacity', '0');
		};
	...
	update: function(element, valueAccessor) {
		...
		if(shell.zoom_in()) {
			...
			_.defer(function() {
				...
				shell.savedStyle = element.style;
		...
		if(shell.fade_out()) {
			...
			_.delay(function() {
				...
				if(game.shell_renders().length > 1)
				{
					var sh = game.shell_renders.shift();
					game.shell_stack.push(sh);
				}
				game.animating_shell(true);
			}, SHELL_ANIM * 1000);
		}
