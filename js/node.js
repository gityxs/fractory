    ...
    double_tap: function(event) {
        ...
        if(self.part()) {
            if(Ice.isa(self.part(), FractalPart)) {
                if(game.animating_shell() || game.dragging_part()) return;
                if(event.shiftKey) {
...
ko.bindingHandlers.draggable = {
	init: function(element, valueAccessor) {
        ...
        // coordinates change as you resize the window, not worth the time figuring out how to keep track of them
        if(opts.snapToNodes)
        {
            var shellHolder = $('.shell_holder');
            var snapPoints = _.chain(game.shell().nodes).map(function(node) {
                return {x: node.pos.x, y: node.pos.y};
            }).value();
            opts.modifiers.push(interact.modifiers.snap(
                {targets: snapPoints, offset: {x: shellHolder.offsetLeft, y: shellHolder.offsetTop}}
            ));
        }
        let draggableElem = interact(element);
        draggableElem.draggable(opts).dragObject = opts.dragObject;
	},
	dispose: function(element, valueAccessor) {
...
ko.bindingHandlers.progressbar = {
    ...
    update: function(element, valueAccessor, allBindingsAccessor, data, context) {
       ...
       // console.log("progressbar update", part.ICEID, loc);;
        ...
        //     'sub2': sub2,
        // }
        // $el.data('progressbar_subs', progressbar_subs);
